...
{
    public class PermissionAppService : CrudAppServiceAsync<Permission, PermissionDto, int>, IPermissionAppService
    {
        ...
        public async Task<PermissionDto> AddPermission(PermissionDto permissionDto)
        {
          return  await AddAsync(async () => {
                var permission = Permission.Create(permissionDto.Name, permissionDto.ServiceName, permissionDto.MethodExecutionName);
                return permission;
            });
        }
